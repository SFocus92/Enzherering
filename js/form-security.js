...
(function() {
    ...
    // Конфигурация
    const YANDEX_SMARTCAPTCHA_SITE_KEY = 'ysc1_8s4L3uIuIGHlXBmULn0VSvYAP9m6MsjkOeNuUJ5o63f8b791'; // Ключ SmartCaptcha
    ...
    function initSmartCaptcha() {
        ...
        // Ищем контейнер (новый с data-sitekey или старый)
        const captchaContainer = document.getElementById('captcha-container') || document.getElementById('smartcaptcha-container');
        if (!captchaContainer) {
            console.warn('Контейнер SmartCaptcha не найден');
            return;
        }

        // Если контейнер имеет data-sitekey, капча инициализируется автоматически
        // Нужно только подписаться на события
        if (captchaContainer.dataset.sitekey) {
            // Подписываемся на событие успешной проверки
            captchaContainer.addEventListener('smartcaptcha:success', function(event) {
                captchaTokenInput.value = event.detail.token;
                submitBtn.disabled = false;
            });
            
            // Подписываемся на событие ошибки
            captchaContainer.addEventListener('smartcaptcha:error', function() {
                captchaTokenInput.value = '';
                submitBtn.disabled = true;
                showNotification('Ошибка загрузки капчи. Пожалуйста, обновите страницу.', 'error');
            });
            
            console.log('SmartCaptcha будет инициализирована автоматически через data-sitekey');
        } else {
            // Ручная инициализация (fallback)
            window.smartCaptcha.render(captchaContainer.id, {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        if (YANDEX_SMARTCAPTCHA_SITE_KEY) {
        ...
        // Отправка на сервер
        try {
            // ВАЖНО: Замените на реальный endpoint вашего сервера
            ...
            // Fallback для локальной разработки (без сервера)
    ...
    // Инициализация капчи после загрузки
    ...
    // Защита от автоматических ботов - honeypot поле
